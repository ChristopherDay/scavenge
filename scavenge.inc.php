        	...
        	if ($this->user->checkTimer("scavenge")) {
	        	...
	        	while ($i < $s["scavenge_rounds"]) {
	        		...
	        		if ($s["scavenge_chance_failed"] >= $spin) {
	        			...
	        			$money += $reward;
	        			$actions[] = array("class" => "success", "text" => "You found $" . number_format($reward));
	        	...
	        	$this->error("After scavenging the streets you found $".number_format($money).", ".number_format($bullets)." bullets and " . number_format($points) . " " . $s["pointsName"], "success");
